...
def set_batchnorm_eval(module):
    """Ensure BatchNorm layers stay in eval mode when feature extracting"""
...
def main():
    ...
    for lr in tqdm(args.lr_list, desc="🔍 Grid Search over Learning Rates"):
        ...
        for epoch in trange(args.epochs, desc=f"LR={lr:.5f}"):
            model.train()
            model.apply(set_batchnorm_eval)  # Re-apply to keep norm layers in eval mode
